...
| Parameter | Description | Default value |
| --- | --- | --- |
| `global.amf.n2if.ipAddress` | The IP address of the AMF’s N2 interface. | `10.47.60.249` |
| `global.n2network.name` | N2 network name. | `n2network` |
| `global.n2network.masterIf` | N2 network MACVLAN master interface. | `eth0` |
| `global.n2network.subnetIP` | N2 network subnet IP address. | `10.47.60.248` |
| `global.n2network.cidr` | N2 network cidr. | `29` |
| `global.n2network.gatewayIP` | N2 network gateway IP address. | `10.47.60.254` |
| `global.n2network.excludeIP` | An exluded IP address from the subnet range. You can give this address to a MACVLAN interface on the host in order to enable to containers to communicate with their host via the N2 interface (optional). | `10.47.60.254` |
...
| Parameter | Description | Default value |
| --- | --- | --- |
| `n3iwf.ike.ipAddress` | The IP address of the N3IWF’s IKE interface. | `172.26.10.5` |
| `n3iwf.ike.name` | IKE network name. | `ikenetwork` |
| `n3iwf.ike.masterIf` | IKE network MACVLAN master interface. | `eth0` |
| `n3iwf.ike.subnetIP` | IKE network subnet IP address. | `172.26.10.0` |
| `n3iwf.ike.cidr` | IKE network cidr. | `24` |
| `n3iwf.ike.gatewayIP` | IKE network gateway IP address. | `172.26.10.1` |
...
| `n3iwf.n2if.ipAddress` | The IP address of the N3IWF's N2 interface. This parameter apply only if `global.amf.service.ngap.enabled` is set to `false`. | `10.47.60.251`|
| `n3iwf.n3if.ipAddress` | The IP address of the N3IWF's N3 interface. | `10.47.60.237`|
| `n3iwf.configuration.IPSecInterfaceAddress` | IPSecInterfaceAddress. | `10.61.0.1`|
